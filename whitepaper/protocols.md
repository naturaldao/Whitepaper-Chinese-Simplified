...
* DAO硬核协议涵括DAO所需要的基础资料如智能体ID、项目英文名称、项目Logo（SVG格式，建议不超过6K，否则实在太贵）、项目描述（可以是宣传口号Slogan）、创建时间（平台记录）、创始成员（匿名，只是ETH地址）、去中心化自治基金的合约地址，等等。
* 该协议可为该DAO的产品与服务提供去中心化认证。
* DAO硬核合约在应用层是可以拓展的。
...
因此关于DAO的准入制度，道易程的构架应该能做到：

* 无需许可、完全开放。
* 如果要对有意加入者进行资质审查，须借助第三方开发的“门卫”模块——这是任何第三方都可以去开发的模块。
...
## **去中心化认证协议 Decentralized Authentication Procotolto** <a href="#deauthentication" id="deauthentication"></a>
...
即：认证标识、DAO的名称、其DAF的合约地址 每家DAO所发行的产品（包括NFT等）和服务下都有该认证标识。 DAO的去中心化认证，可作为DAO的状态（如创世和休眠）的证据，解决智能体及其发行的代币和产品（如DApp）的真实信息的证明等问题。 DApp的去中心化认证，除了解决DApp真实信息的证明之外，还会解决DAism接口调用等需求。DApp获得DAism的认证后（完成注册），道易程之上的任何DAO都可以通过DAism Store的接口搜索和购买其服务或产品。
