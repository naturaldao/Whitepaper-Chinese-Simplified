...
3. 去中心化交易协议（Dex Protocol）\
   估值通证（代币）全部发行到 IADD 网络后成为储备币（为市场构建好流动性池）。储备币通过自动计算而实现价值发现，并为市场提供流动性，且不必考虑交易规模大小。\
   这是一个革命性的协议，因为基于它而构建的去中心化交易网络不需要也不允许任何人参与流动性！
4. 通用支付协议（Protocol of Universal Payment）\
   该协议的重点是提供支付专用的接口，并且最终完成对以太坊gas的支付的改进。
...
* 每家DAO的估值通证都全部发行到了 IADD 网络， IADD 里的估值通证的价格能够反应市场的真实评判。
...
2. IADD 网络利用与Uniswap类似的策略，但会以计量单位通证代替ETH来计算估值通证的价格。
...
5. 计量单位通证的另一个重要的应用领域是去中心化结算。因其它一切代币的价格都是波动的，无法被消费者持有而用于日常消费，只有uToken价格恒定（1 uToken ≡ 1 vita），因此它是道易程生态里合格的通用支付币种（或称结算币种）。
...
1. 锻造初始化\
   协议启动（即合约启用）时，算法以每个计量单位通证所定义的价格即1 vita、ETH历史最高价（4086美元）为基础进行初始化，即 1 ETH 可以锻造出4086个uToken。
