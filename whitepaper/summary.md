...
**道易程是以太坊通证经济和DAO的价值体系的公共治理层。价值证明（PoV，Proof of Value）是其独特的治理共识。道易程的使命是让通证经济得以蓬勃发展，让 DAO暨dApp 得以繁衍生息。**
...
* DAO = 价值证明 + 智能合约 + 专属估值通证\
  道易程的价值证明是DAO得到有效治理的保障。dApp的智能合约的价值是DAO的根本。而在道易程构建起来的良好的通证经济里，专属估值通证化身为dApp的价值评估指标。
...
2. 以奖励为驱动力\
   每笔价值通证交易需缴纳2%的手续费，并奖励给对应的DAO的开发者。这种奖励机制可以驱动所有的dApp成为智能公物（Smart Commons）。
3. 驱动所有的dApp成为智能公物（Smart Commons/Public Goods）\
   Mint DAO意味着开发者自愿将其开发的dApp转为智能公物（Smart Commons，即Public Goods的一种）。这一协议（特别是其子协议SC0）符合 DAism 对培育公共资源/公共品和开放协作生态系统的承诺。
